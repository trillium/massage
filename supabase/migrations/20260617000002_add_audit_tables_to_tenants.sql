-- Backfill contact_submissions + email_sends for existing tenants
-- (trillium_massage already has these from 20260617000001)

DO $$
DECLARE
  t TEXT;
BEGIN
  FOREACH t IN ARRAY ARRAY['sarah_music']
  LOOP
    -- contact_submissions
    EXECUTE format($sql$
      CREATE TABLE IF NOT EXISTS %I.contact_submissions (
    ...
    $sql$, t);
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_contact_email      ON %I.contact_submissions(email)',      t, t);
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_contact_created_at ON %I.contact_submissions(created_at)', t, t);
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_contact_state      ON %I.contact_submissions(send_state)', t, t);
    EXECUTE format('ALTER TABLE %I.contact_submissions ENABLE ROW LEVEL SECURITY', t);
    BEGIN
      EXECUTE format('CREATE POLICY "Service role full access on contact_submissions" ON %I.contact_submissions FOR ALL USING (auth.role() = ''service_role'')', t);
    EXCEPTION WHEN duplicate_object THEN NULL;
    END;

    -- email_sends
    EXECUTE format($sql$
      CREATE TABLE IF NOT EXISTS %I.email_sends (
        id           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    $sql$, t);
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_email_sends_template   ON %I.email_sends(template)',   t, t);
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_email_sends_to         ON %I.email_sends(to_address)', t, t);
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_email_sends_created_at ON %I.email_sends(created_at)', t, t);
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_email_sends_state      ON %I.email_sends(send_state)', t, t);
    EXECUTE format('ALTER TABLE %I.email_sends ENABLE ROW LEVEL SECURITY', t);
    BEGIN
      EXECUTE format('CREATE POLICY "Service role full access on email_sends" ON %I.email_sends FOR ALL USING (auth.role() = ''service_role'')', t);
    EXCEPTION WHEN duplicate_object THEN NULL;
    END;
  END LOOP;
END;
...
-- Update create_tenant() to include contact_submissions and email_sends for future tenants
...
BEGIN
  ...
  BEGIN EXECUTE format('CREATE POLICY "Service role full access on google_credentials" ON %I.google_credentials FOR ALL USING (auth.role() = ''service_role'')', p_tenant_slug);
  ...
  BEGIN EXECUTE format('CREATE POLICY "Service role full access on appointments" ON %I.appointments FOR ALL USING (auth.role() = ''service_role'')', p_tenant_slug);
  EXCEPTION WHEN duplicate_object THEN NULL; END;
  ...
  BEGIN EXECUTE format('CREATE POLICY "Service role full access on reminders" ON %I.reminders FOR ALL USING (auth.role() = ''service_role'')', p_tenant_slug);
  EXCEPTION WHEN duplicate_object THEN NULL; END;
  ...
  BEGIN EXECUTE format('CREATE POLICY "Service role full access on reminder_logs" ON %I.reminder_logs FOR ALL USING (auth.role() = ''service_role'')', p_tenant_slug);
  EXCEPTION WHEN duplicate_object THEN NULL; END;
  ...
  BEGIN EXECUTE format('CREATE POLICY "Service role full access on invoices" ON %I.invoices FOR ALL USING (auth.role() = ''service_role'')', p_tenant_slug);
  EXCEPTION WHEN duplicate_object THEN NULL; END;
  ...
  BEGIN EXECUTE format('CREATE POLICY "Service role full access on invoice_audit_log" ON %I.invoice_audit_log FOR ALL USING (auth.role() = ''service_role'')', p_tenant_slug);
  EXCEPTION WHEN duplicate_object THEN NULL; END;
  ...
  BEGIN EXECUTE format('CREATE POLICY "Service role full access on slot_holds" ON %I.slot_holds FOR ALL USING (auth.role() = ''service_role'')', p_tenant_slug);
  EXCEPTION WHEN duplicate_object THEN NULL; END;
  BEGIN EXECUTE format('CREATE POLICY "Anon can read slot_holds" ON %I.slot_holds FOR SELECT USING (auth.role() = ''anon'')', p_tenant_slug);
  EXCEPTION WHEN duplicate_object THEN NULL; END;
  ...
  BEGIN EXECUTE format('CREATE POLICY "Service role full access on reviews" ON %I.reviews FOR ALL USING (auth.role() = ''service_role'')', p_tenant_slug);
  EXCEPTION WHEN duplicate_object THEN NULL; END;
  BEGIN EXECUTE format('CREATE POLICY "Anon can read reviews" ON %I.reviews FOR SELECT USING (auth.role() = ''anon'')', p_tenant_slug);
  EXCEPTION WHEN duplicate_object THEN NULL; END;
  ...
  BEGIN EXECUTE format('CREATE POLICY "Service role full access on admin_emails" ON %I.admin_emails FOR ALL USING (auth.role() = ''service_role'')', p_tenant_slug);
  EXCEPTION WHEN duplicate_object THEN NULL; END;
  ...
  BEGIN EXECUTE format('CREATE POLICY "Service role full access on raffles" ON %I.raffles FOR ALL USING (auth.role() = ''service_role'')', p_tenant_slug);
  EXCEPTION WHEN duplicate_object THEN NULL; END;
  BEGIN EXECUTE format('CREATE POLICY "Anon can read raffles" ON %I.raffles FOR SELECT USING (auth.role() = ''anon'')', p_tenant_slug);
  EXCEPTION WHEN duplicate_object THEN NULL; END;
  ...
  BEGIN EXECUTE format('CREATE POLICY "Service role full access on raffle_entries" ON %I.raffle_entries FOR ALL USING (auth.role() = ''service_role'')', p_tenant_slug);
  EXCEPTION WHEN duplicate_object THEN NULL; END;
  BEGIN EXECUTE format('CREATE POLICY "Anon can insert raffle_entries" ON %I.raffle_entries FOR INSERT WITH CHECK (auth.role() = ''anon'')', p_tenant_slug);
  EXCEPTION WHEN duplicate_object THEN NULL; END;
  ...
  BEGIN EXECUTE format('CREATE POLICY "Service role full access on sandbox_sessions" ON %I.sandbox_sessions FOR ALL USING (auth.role() = ''service_role'')', p_tenant_slug);
  EXCEPTION WHEN duplicate_object THEN NULL; END;
  ...
  $sql$, p_tenant_slug);
  EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_contact_email      ON %I.contact_submissions(email)',      p_tenant_slug, p_tenant_slug);
  EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_contact_created_at ON %I.contact_submissions(created_at)', p_tenant_slug, p_tenant_slug);
  EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_contact_state      ON %I.contact_submissions(send_state)', p_tenant_slug, p_tenant_slug);
  EXECUTE format('ALTER TABLE %I.contact_submissions ENABLE ROW LEVEL SECURITY', p_tenant_slug);
  BEGIN EXECUTE format('CREATE POLICY "Service role full access on contact_submissions" ON %I.contact_submissions FOR ALL USING (auth.role() = ''service_role'')', p_tenant_slug);
  EXCEPTION WHEN duplicate_object THEN NULL; END;
  ...
  $sql$, p_tenant_slug);
  EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_email_sends_template   ON %I.email_sends(template)',   p_tenant_slug, p_tenant_slug);
  EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_email_sends_to         ON %I.email_sends(to_address)', p_tenant_slug, p_tenant_slug);
  EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_email_sends_created_at ON %I.email_sends(created_at)', p_tenant_slug, p_tenant_slug);
  EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_email_sends_state      ON %I.email_sends(send_state)', p_tenant_slug, p_tenant_slug);
  EXECUTE format('ALTER TABLE %I.email_sends ENABLE ROW LEVEL SECURITY', p_tenant_slug);
  BEGIN EXECUTE format('CREATE POLICY "Service role full access on email_sends" ON %I.email_sends FOR ALL USING (auth.role() = ''service_role'')', p_tenant_slug);
  EXCEPTION WHEN duplicate_object THEN NULL; END;
