  ...
  return (
    <Box>
      <H4 className="mb-3 flex items-center dark:text-white">
        <Box className={`mr-2 h-3 w-3 rounded-full ${dotColor}`}></Box>
      ...
      {events.length === 0 ? (
        ...
        <Box className="space-y-2">
          {events.map((event) => (
            <Box key={event.id} className={`rounded-md border ${borderColor} p-3 text-sm`}>
              <Box className="font-medium text-accent-900 dark:text-white">{event.summary}</Box>
              <Box className="mt-1 text-accent-600 dark:text-accent-400">
                {formatDateTime(event.start.dateTime)}
                {admin.activeEventContainers.eventTimesSeparator}
                {formatDateTime(event.end.dateTime)}
              </Box>
              {event.description && (
                <Box className="mt-1 text-xs text-accent-500 dark:text-accent-500">
